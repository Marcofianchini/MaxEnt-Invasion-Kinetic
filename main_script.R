...
# DATA TO REPRODUCE MODEL SELECTION AND PERFORMANCE METRICS FROM SCRATCH (BE CAREFUL, IT MAY TAKE A LOT OF TIME EVEN WITH HIGH PARALLELIZATION)
file.edit(paste0(wd,'scripts/','model_tuning.R'))

# DATA TO REPRODUCE MODEL SELECTION AND PERFORMANCE METRICS (starts from ENMevaluate object, containing all the fitted models)
# load ENMevaluate object
e.mx <- readRDS(paste0(wd,'data/','e_mx.RDS'))   
...
########### VARIABLE IMPORTANCE 
source(paste0(wd,'scripts/','compute_variable_importance.R'))


########### PERFORMANCE METRICS & RANKING 
source(paste0(wd,'scripts/','performance_and_ranking.R'))


########### MOBILITY ORIENTED PARITY ANALYSIS 
source(paste0(wd,'scripts/','compute_mop_percentile_maps.R'))


########### RESPONSE CURVES
source(paste0(wd,'scripts/','compute_response_curves.R'))


########### PRESENT AND FUTURE PDF CURVES
nCores <- 5 #set appropriate core numbers to parallelize the processing. Quite long process
source(paste0(wd,'scripts/','compute_pdfs.R'))


########### FUTURE PROJECTIONS
source(paste0(wd,'scripts/','compute_future_projections.R'))
